...
func generateDataSqLite() {
    ...
    var querySQL = "DROP TABLE sensors; DROP TABLE readings;"
    sqlite3_exec(db, querySQL, nil, nil, nil)
    
    querySQL = "CREATE TABLE sensors (id INTEGER PRIMARY KEY AUTOINCREMENT, name VARCHAR(50), desc VARCHAR(50));"
    sqlite3_exec(db, querySQL, nil, nil, nil)
    ...
    for i in 1...20 {
        querySQL = "INSERT INTO sensors (name, desc) VALUES ('\(String(format: "S%02d", i))', 'Sensor number \(i)')"
        sqlite3_exec(db, querySQL, nil, nil, nil)
    }

    for _ in 1...ViewController.numberOfReadings {
        querySQL = "INSERT INTO readings (timestamp, value, sensor) VALUES (\(Int.random(in: 1546300800 ... 1577836799)), \(Float.random(in: 0 ... 100)), \(Int.random(in: 1 ... 20)) )"
        sqlite3_exec(db, querySQL, nil, nil, nil)
    }
